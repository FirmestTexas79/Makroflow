...
// v rozsahu 6–12 opakování s rezervou 0–RIRmax, denní forma ±3 %, únava −3 % na sérii,
...
fun main() {
    println("RIR | tréninků | model | poslední trénink (Epley) | max. dosavadní (Epley) | pokrytí 95% intervalu")
    for (maxRir in listOf(2, 3)) for (n in listOf(4, 8, 16)) {
        ...
        repeat(3000) {
            ...
            for (i in 0 until n) {
                ...
                repeat(3) { k ->
                    ...
                    if (reps >= 1) sets += LoggedSet(day = d, exerciseId = "x", weightKg = load, reps = reps, slowEccentric = slow)
                }
            }
        ...
        println("0–%d | %2d | %.2f %% | %.2f %% | %.2f %% | %.0f %%".format(maxRir, n, 100 * eM / runs, 100 * eL / runs, 100 * eX / runs, 100.0 * cov / runs))
    }
}
