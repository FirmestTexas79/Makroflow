...
class SnackFragment : Fragment() {
    ...
    private fun displaySnacks(allSnacks: List<SnackEntity>) {
        ...
        filtered.forEach { snack ->
            ...
            val wP = ((snack.p / globalCeiling) * 30f).coerceAtLeast(0f)
            val wS = ((snack.s / globalCeiling) * 30f).coerceAtLeast(0f)
            val wT = ((snack.t / globalCeiling) * 30f).coerceAtLeast(0f)
    ...
    private fun setupMacroBar(bar: View, value: Float, max: Float) {
        ...
        // weightSum = 100, každý bar dostane svůj podíl z max. 30 jednotek (zbytek = barEmpty)
        params.weight = if (value > 0) ((value / max) * 30f) else 0f
        bar.layoutParams = params
    }

    private fun updateBarEmpty(card: View, barPWeight: Float, barSWeight: Float, barTWeight: Float) {
        val barEmpty = card.findViewById<View>(R.id.barEmpty)
        val params = barEmpty.layoutParams as LinearLayout.LayoutParams
        params.weight = (100f - barPWeight - barSWeight - barTWeight).coerceAtLeast(0f)
        barEmpty.layoutParams = params
    }
    ...
    private fun startBarcodeScanner(
        ...
        etP: EditText, etS: EditText, etT: EditText
    ) {
        val scanner = GmsBarcodeScanning.getClient(requireContext(), GmsBarcodeScannerOptions.Builder().build())
        scanner.startScan().addOnSuccessListener { barcode ->
            barcode.rawValue?.let { fetchFoodData(it, etName, etWeight, etP, etS, etT) }
        }
    }
    ...
    private fun setupSwitchColors(switch: SwitchMaterial) {
