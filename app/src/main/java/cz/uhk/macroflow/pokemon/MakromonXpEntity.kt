...
/**
 * XP systém pro Makromony na liště.
 *
 ...
 *   +6  XP — splněný denní krokový cíl
 *
 ...
 *   Lv10 → 3500 XP
 */
@Entity(tableName = "pokemon_xp")  // název tabulky zachován pro zpětnou kompatibilitu DB
data class MakromonXpEntity(
    @PrimaryKey val makromonId: String,     // Např. "012" (Spirra)
    val totalXp: Int = 0,
    val lastDailyRewardDate: String = ""    // "yyyy-MM-dd" – zabraňuje více odměnám za den
)

@Dao
interface MakromonXpDao {

    @Query("SELECT * FROM pokemon_xp WHERE makromonId = :makromonId LIMIT 1")
    fun getXp(makromonId: String): MakromonXpEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun setXp(entity: MakromonXpEntity)

    @Transaction
    fun addXp(makromonId: String, amount: Int) {
        val current = getXp(makromonId) ?: MakromonXpEntity(makromonId)
        setXp(current.copy(totalXp = current.totalXp + amount))
    }

    @Query("DELETE FROM pokemon_xp WHERE makromonId = :makromonId")
    fun deleteXp(makromonId: String)
}
...
/** XP odměny za splněné cíle */
object XpRewards {
    const val DAILY_OPEN      = 2   // otevření aplikace každý den
    const val LOGGED_FOOD     = 5   // zalogováno alespoň 3 jídla
